...
classdef flexRod < handle
    ...
    methods
       %% 构造函数
        function item = flexRod(E,l_inital,w,h,n,pos1,pos2)
        % calculate necessary properties at initialization
            item.E=E;
            item.Ltotal=l_inital;
            item.wid=w;
            item.thi=h;
            ...
            item.conv_K_theta=diag((item.E*item.Iz/item.seg_length)*ones(n,1));
        end
       ...
       %% 初始化conv参数
        function init_exp(obj)
            ...
            obj.conv_K_theta=obj.cal_K_theta;
            obj.conv_K_J=-obj.partial_J_theta(obj.conv_w_all,obj.conv_q_all,obj.conv_jacobs,obj.conv_F);
            obj.conv_K_L=obj.cal_K_L;
            obj.conv_J_L=obj.cal_J_L(obj.conv_w_all,obj.conv_q_all);
        end
        
        
       %% 更新conv下的参数
        % 根据新的Ltotal,conv_theta,conv_F来计算参数
...
%             g=obj.exp_fkine(obj.conv_w_all,obj.conv_q_all,obj.conv_g0,obj.conv_theta); 
            ...
            tau=obj.conv_K_theta*obj.conv_theta-transpose(obj.conv_jacobs)*obj.conv_F;
       ...
       %% 计算雅可比矩阵 在conv坐标系下
        function JACOBIANs=cal_Jacob_conv(obj)
            ...
            JACOBIANs(end,end-5:end)=transpose(dir);
        end
        
        function Ktheta=cal_K_theta(obj)
        ...
        function JL=cal_J_L(obj,w_all,q_all)
            Theta=obj.conv_theta;
            n=obj.n_seg;

            xihat_all=obj.hight(w_all,q_all);
            xi_all=[-obj.cha(w_all,q_all);w_all];
            ...
            xi_n(1:3,1)=xihat_n(1:3,4);
            xi_n(4,1)=-xihat_n(2,3);
            xi_n(5,1)=xihat_n(1,3);
            xi_n(6,1)=-xihat_n(1,2);
            ...
            for i=1:n+1
                ...
                if i == n+1
                   partial_xi_l=[1;0;0;0;0;0];
                else
                   partial_xi_l=[(i-0.5);0;0;0;0;0];
                end

                JL=JL+H*A*partial_xi_l;        
            end
        end
