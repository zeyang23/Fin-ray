% 有3根柔性支撑的finray，作用三角函数线载荷

% 三角函数线载荷采用全局坐标系，不与状态变量耦合
...
press_series=linspace(press_s,press_e,101);
...
for i=1:length(press_series)
    ...
    options_D = optimset('Algorithm','trust-region-reflective');
    [x_series(:,i),fval_cosserat,exitflag_cosserat,output_cosserat]=fsolve(f,x0,options_B);
%     [x_series(:,i),~,~,exitflag_cosserat,~]=lsqnonlin(f,x0);
    
%     g=@(x) norm(f(x));
%     [x_series(:,i),~,exitflag_cosserat,~]=fmincon(g,x0);
