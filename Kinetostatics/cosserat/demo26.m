...
delta_series=linspace(delta,delta,1);
...
for i=1:length(delta_series)
    ...
    pA(1)=xA-delta_series(i);
    pB(1)=xB-delta_series(i);
    ...
    [x_series(:,i),fval_cosserat,exitflag_cosserat,output_cosserat]=fsolve(f,x0,options_A);
...
L_start=x_cosserat(18);
theta1=x_cosserat(19);
theta2=x_cosserat(20);
fp=x_cosserat(21);
...
[sA_1,YA_1] = ode45(@(s,y) get_ydot_A(s,y,L_start,L_start+radius*(theta2-theta1),fp,LA,EA,IA), span_a1, ya1_0,options_a);
...
[sA_2,YA_2] = ode45(@(s,y) get_ydot_A(s,y,L_start,L_start+radius*(theta2-theta1),fp,LA,EA,IA), span_a2, ya2_0,options_a);
ya_2=transpose(YA_2(end,:));

FA_2=[-fcon2*cos(gamma2);-fcon2*sin(gamma2)];
span_a3 = [lambdaA2,lambdaA3];
ya3_0=ya_2+[0;0;0;-FA_2;0];
[sA_3,YA_3] = ode45(@(s,y) get_ydot_A(s,y,L_start,L_start+radius*(theta2-theta1),fp,LA,EA,IA), span_a3, ya3_0,options_a);
ya_3=transpose(YA_3(end,:));

FA_3=[-fcon3*cos(gamma3);-fcon3*sin(gamma3)];
span_a4 = [lambdaA3,lambdaA4];
ya4_0=ya_3+[0;0;0;-FA_3;0];
[sA_4,YA_4] = ode45(@(s,y) get_ydot_A(s,y,L_start,L_start+radius*(theta2-theta1),fp,LA,EA,IA), span_a4, ya4_0,options_a);
ya_4=transpose(YA_4(end,:));

FA_4=[-fcon4*cos(gamma4);-fcon4*sin(gamma4)];
span_a5 = [lambdaA4,LA];
ya5_0=ya_4+[0;0;0;-FA_4;0];
[sA_5,YA_5] = ode45(@(s,y) get_ydot_A(s,y,L_start,L_start+radius*(theta2-theta1),fp,LA,EA,IA), span_a5, ya5_0,options_a);
...
press_start_x=interp1(sA_total,YA_total(:,1),L_start);
press_start_y=interp1(sA_total,YA_total(:,2),L_start);
press_end_x=interp1(sA_total,YA_total(:,1),L_start+radius*(theta2-theta1));
press_end_y=interp1(sA_total,YA_total(:,2),L_start+radius*(theta2-theta1));
...
function res=check_balance(x,problem_info,psi,pA,pB,LA,LB,EA,EB,IA,IB)
    ...
    res(18)=xc+radius*sin(theta1)-press_start_x;
    res(19)=yc-radius*cos(theta1)-press_start_y;
    res(20)=xc+radius*sin(theta2)-press_start_x;
    res(21)=yc-radius*cos(theta2)-press_start_y;
end
