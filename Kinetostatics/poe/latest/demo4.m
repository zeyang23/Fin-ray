% 使用指数坐标法求解有一根刚性支撑、有一个外力的Fin-ray
% 手写牛顿法
...
L_F=0.8*LA;
NF=fix((L_F-seg_length_A/2)/seg_length_A)+1;
fp=2; % 法向力的大小
...
g0_Fout=[rotz(alpha_degree),[xA;yA;0]+L_F*[cos(alpha);sin(alpha);0];[0,0,0,1]];
...
problem_info.L_F=L_F;
problem_info.NF=NF;
problem_info.fp=fp;
problem_info.g0_Fout=g0_Fout;
...
function [res,J]=cal_balance(x,problem_info)
    ...
    J_Fout=zeros(size(Js_A));
    J_Fout(:,1:NF)=Js_A(:,1:NF);
    ...
    g_Fout=FKinSpace(problem_info.g0_Fout,problem_info.Slist_A(:,1:NF),theta_A(1:NF));
    
    r1=[eye(3),zeros(3,1)]*g_F1*[0;0;0;1];
    r2=[eye(3),zeros(3,1)]*g_F2*[0;0;0;1];
    
%     rF=[eye(3),zeros(3,1)]*g_Fout*[0;0;0;1];
    
    Fout_tool=[0;0;0;0;-problem_info.fp;0];
    
    Fout=transpose(Adjoint(TransInv(g_Fout)))*Fout_tool;
    
    
    F1=[cross(r1,f1);f1];
    F2=[cross(r2,f2);f2];
    
    res2=problem_info.K_A*theta_A-transpose(Js_A)*Fe-transpose(J_F1)*F1-transpose(J_Fout)*Fout;
    ...
    KF_out=zeros(nA);
    for i=1:NF
        KF_out(:,i)=-transpose(J_Fout)*transpose(ad(J_Fout(:,i)))*Fout;
    end

    
    J(7:nA+6,1:nA)=problem_info.K_A-get_partial(Js_A,Fe)-get_partial(J_F1,F1)-KF_a-get_partial(J_Fout,Fout)-KF_out;
