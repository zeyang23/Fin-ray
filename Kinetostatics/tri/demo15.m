...
k_constant=fix(1/2*nA);
LambdaK=zeros(k_constant,nA);
LambdaK(1:k_constant,1:k_constant)=diag(ones(k_constant,1));
...
RA_F=planar_nR(E,k_constant/nA*LA,wid,thi,k_constant,pdes);
...
f=@(x) myfun(x,xA,xB,yA,yB,beta,beta_degree,alpha,alpha_degree,A,B,b,nA,nB,F_constant,RA,RB,RA_F,RAk1,RAk2,RAk3,RAk4,RBk1,RBk2,RBk3,RBk4,Lcon1,Lcon2,Lcon3,Lcon4,k_constant,ka1,ka2,ka3,ka4,kb1,kb2,kb3,kb4,LambdaK,LambdaA1,LambdaA2,LambdaA3,LambdaA4,LambdaB1,LambdaB2,LambdaB3,LambdaB4);
...
%%
function [r,J]=myfun(x,xA,xB,yA,yB,beta,beta_degree,alpha,alpha_degree,A,B,b,nA,nB,F_constant,RA,RB,RA_F,RAk1,RAk2,RAk3,RAk4,RBk1,RBk2,RBk3,RBk4,Lcon1,Lcon2,Lcon3,Lcon4,k_constant,ka1,ka2,ka3,ka4,kb1,kb2,kb3,kb4,lambdaK,lambdaA1,lambdaA2,lambdaA3,lambdaA4,lambdaB1,lambdaB2,lambdaB3,lambdaB4)
    
    thetaA=zeros(nA,1);
    thetaA_F=zeros(k_constant,1);
    ...
    pk_F=RA_F.pe;
    RA_F.F=[F_constant*sin(pk_F(3));-F_constant*cos(pk_F(3));0];
    RA_F.update;
    ...
    r2=RA.K_theta*thetaA-transpose(RA.Jacobian)*FA-...
       ...
       transpose(lambdaK)*transpose(RA_F.Jacobian)*[F_constant*sin(pk_F(3));-F_constant*cos(pk_F(3));0];
    ...
    if nargout > 1
        ...
        % 外力带来的两项
        tempF1=transpose(lambdaK)*RA_F.partial*lambdaK;
        tempF2=transpose(lambdaK)*transpose(RA_F.Jacobian)*[F_constant*cos(pk_F(3));F_constant*sin(pk_F(3));0]*ones(1,k_constant)*lambdaK;
        J(4:nA+3,1:nA)=RA.K_theta-1*(1*tempA0+tempA1+tempA2+tempA3+tempA4+tempF1+tempF2);
