...
%             JACOBIANs(1:6,1:obj.n_seg)=obj.adjoint(inv(obj.conv_gt))*obj.conv_jacobs;
       ...
       %% FABRIK算法
        function FABRIK_solver(obj,tol)
            obj.cal_pos_all();
            
            base=transpose(obj.conv_pos_all(:,2));
            target=[obj.conv_end_pos(1),obj.conv_end_pos(2)];
            
            d=obj.seg_length*ones(obj.n_seg,1);
            
            p0=transpose(obj.conv_pos_all(:,2:end));
            
            p_solve=obj.simpFABRIK(base,target,d,p0,tol);
            
            p=[0,0;p_solve];
            obj.conv_theta=obj.p2theta(p);
            obj.conv_g=obj.exp_fkine(obj.conv_w_all,obj.conv_q_all,obj.conv_g0,obj.conv_theta);
        end
        
        function theta=p2theta(obj,p)
        ...
        function p_solve=simpFABRIK(obj,base,target,d,p0,tol)
            k=0;
            ...
            while(1)
                ...
                elseif k>1000
                    ...
                    p=obj.forward(p,d,target);
                    p=obj.backward(p,d,base);
        ...
        function p_new=forward(obj,p,d,target)
            n=length(d)+1;
            p_new=p;
            p_new(end,:)=target;
        ...
        function p_new=backward(obj,p,d,base)
            n=length(d)+1;
            p_new=p;
            p_new(1,:)=base;
